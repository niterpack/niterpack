use std::env;
use eyre::{ContextCompat, ensure, eyre, WrapErr};
use log::info;
...
impl AddArgs {
    pub fn mod_data(&self) -> eyre::Result<Mod> {
        ...
        let version_id = match &self.version_name {
            Some(version_name) =>
                match modrinth::get_version(version_name).wrap_err("failed to fetch modrinth version")? {
                    Some(version) => version.id,
                    None => modrinth::get_versions(&self.mod_name)
